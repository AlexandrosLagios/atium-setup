...
- The no-API-key rule and the Gemini opt-in
- Part A: structural extraction from code, with the AST pass
- Part B: semantic extraction for docs, papers, and images, with the subagent
  chunking protocol and the cache contract
- Part C: the merge, with AST nodes first and semantic nodes deduplicated by id

Return to `SKILL.md` at Step 4 when the merge has written
