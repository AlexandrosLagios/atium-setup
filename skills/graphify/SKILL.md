...
Two parts: structural extraction (deterministic, free) and semantic extraction
(LLM, costs tokens). The full runbook is `references/extraction.md`, covering
Part A (the AST pass), Part B (the semantic subagents, the chunking protocol, and
the cache contract), and Part C (the merge). `references/extraction-spec.md`
carries the node and edge schema those parts write. Read both before extracting,
then continue at Step 4.

Two rules travel with the step, so they stay here:

- **graphify needs no API key.** Never ask the user for one, and never block on
  one. Code is extracted structurally with no LLM, so a code-only corpus skips
  semantic extraction entirely. Semantic extraction uses Gemini only when
  `GEMINI_API_KEY` or `GOOGLE_API_KEY` is already set, and otherwise the host
  agent is the LLM. graphify never reads `ANTHROPIC_API_KEY` or `OPENAI_API_KEY`.
- **`--mode deep` must survive.** Pass `DEEP_MODE=true` to every subagent when
  the original invocation carried the flag.
